...
class VolaSupabaseUser extends BaseAuthUser {
  VolaSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> volaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = VolaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
